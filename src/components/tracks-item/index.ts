...
export function createTrackItem(track: Track, index: number): string {
  return `
    <li class="tracks__item flex">
      <div class="tracks__item__number">${index + 1}</div>
      <div class="tracks__item__name"><img class="track__img" src="${track.picture}" alt="${track.name}">
        <div class="track__content">
          <h3 class="track__name">
            <a class="track__name__link" href="#">${track.name}</a>
          </h3>
      ...
      <div class="tracks__item__data flex">
        <span class="data__text">${track.dateAdded}</span>
        <button class="track__like-btn ${track.isLiked ? 'like-btn--active' : ""}">
